...
TORCH_LIBRARY_FRAGMENT(sgl_kernel, m) {
  ...
  m.def(
      "layerkv_copy_kv_contiguous_batched(Tensor[] src_ks, Tensor[] src_vs, Tensor[] dst_ks, Tensor[] dst_vs, "
      "Tensor src_base_slots, Tensor dst_base_slots, Tensor token_counts, int item_size) -> ()");
  m.impl("layerkv_copy_kv_contiguous_batched", torch::kCUDA, &layerkv_copy_kv_contiguous_batched);
  m.def(
